...
use pluginop_common::{Input, quic::ConnectionId};
...
extern "C" {
    ...
    fn call_proto_op_from_plugin(po_ptr: u32, po_len: u32, po_args_ptr: u32,
        po_args_len: u32, po_input_ptr: u32, po_input_len: u32, po_res_ptr: u32, po_res_len: u32);
    /* Gets a connection field */
    fn get_connection_from_plugin(field_ptr: u32, field_len: u32, res_ptr: u32, res_len: u32) -> i64;
    ...
    fn get_sent_packet_from_plugin(field_ptr: u32, field_len: u32, res_ptr: u32, res_len: u32) -> i64;
    /* Get a received packet field */
    fn get_rcv_packet_from_plugin(field_ptr: u32, field_len: u32, res_ptr: u32, res_len: u32) -> i64;
    ...
    T: Serialize,
{
    ...
    unsafe { save_output_from_plugin(serialized_value.as_ptr() as u32, serialized_value.len() as u32) }
}
...
pub fn buffer_get_bytes(b: &mut Vec<u8>) -> i64 {
    unsafe { buffer_get_bytes_from_plugin(b.as_mut_slice() as *mut [u8] as *mut u8 as u32, b.len() as u32) }
}
    ...
    T: Deserialize<'de>,
{
    ...
    unsafe {
       ...
       get_connection_from_plugin(serialized_field.as_ptr() as u32, serialized_field.len() as u32,
                                   res.as_mut_ptr() as u32, SIZE as u32);
    }
    ...
    T: Serialize,
{
    ...
    let serialized_value = bincode::serialize(&v).expect("serialized value");
    unsafe {
        set_connection_from_plugin(serialized_field.as_ptr() as u32, serialized_field.len() as u32,
                                   serialized_value.as_ptr() as u32, serialized_value.len() as u32);
    }
}
    ...
    T: Deserialize<'de>,
{
    ...
    let mut res = Vec::<u8>::with_capacity(SIZE).into_boxed_slice();
    unsafe {
        get_recovery_from_plugin(serialized_field.as_ptr() as u32, serialized_field.len() as u32,
                                   res.as_mut_ptr() as u32, SIZE as u32);
    }
    let slice = unsafe { std::slice::from_raw_parts(res.as_ptr(), SIZE as usize) };
    ...
    T: Serialize,
{
    ...
    let serialized_value = bincode::serialize(&v).expect("serialized value");
    unsafe {
        set_recovery_from_plugin(serialized_field.as_ptr() as u32, serialized_field.len() as u32,
                                 serialized_value.as_ptr() as u32, serialized_value.len() as u32);
    }
}
    ...
    T: Deserialize<'de>,
{
    ...
    unsafe {
        // FIXME we should handle error.
        get_sent_packet_from_plugin(serialized_field.as_ptr() as u32, serialized_field.len() as u32, res.as_mut_ptr() as u32, SIZE as u32);
    }
    let slice = unsafe { std::slice::from_raw_parts(res.as_ptr(), SIZE as usize) };
    bincode::deserialize(slice).expect("no error")
}
    ...
    T: Deserialize<'de>,
{
    ...
    unsafe {
        // FIXME we should handle error.
        get_rcv_packet_from_plugin(serialized_field.as_ptr() as u32, serialized_field.len() as u32, res.as_mut_ptr() as u32, SIZE as u32);
    }
    let slice = unsafe { std::slice::from_raw_parts(res.as_ptr(), SIZE as usize) };
    bincode::deserialize(slice).expect("no error")
}
...
pub fn call_protoop(po: ProtoOp, args: Vec<PluginVal>, inputs: Vec<Input>) -> Vec<PluginVal> {
    ...
    unsafe {
        call_proto_op_from_plugin(
            serialized_po.as_ptr() as u32, serialized_po.len() as u32, serialized_args.as_ptr() as u32,
            serialized_args.len() as u32, serialized_inputs.as_ptr() as u32,
            serialized_inputs.len() as u32, res.as_mut_ptr() as u32, SIZE as u32);
    }
    let slice = unsafe { std::slice::from_raw_parts(res.as_ptr(), SIZE as usize) };
    bincode::deserialize(slice).expect("no error")
}
...
pub fn get_current_time() -> Instant {
    let mut res = Vec::<u8>::with_capacity(SIZE).into_boxed_slice();
    unsafe { get_current_time_from_plugin(res.as_mut_ptr() as u32, SIZE as u32); }
    let slice = unsafe { std::slice::from_raw_parts(res.as_ptr(), SIZE as usize) };
    bincode::deserialize(slice).expect("no error")
}
...
pub fn register(r: quic::Registration) {
    ...
    unsafe { register_from_plugin(serialized_field.as_ptr() as u32, serialized_field.len() as u32) }
}
...
pub fn generate_connection_id() -> Option<ConnectionId> {
    let mut res = Vec::<u8>::with_capacity(SIZE).into_boxed_slice();
    let err = unsafe {
        generate_connection_id_from_plugin(res.as_mut_ptr() as u32, SIZE as u32)
    };
    if err != 0 {
        return None;
    }
...
pub fn set_timer(ts: unix_time::Instant, id: u64, callback_name: &str) -> u64 {
    ...
    unsafe {
        set_timer_from_plugin(
            serialized_ts.as_ptr() as u32, serialized_ts.len() as u32,
            id,
            serialized_cb.as_ptr() as u32, serialized_cb.len() as u32,
        )
    }
}
